...
const ProfessionalSkillSection = () => {
  ...
  return (
    <div className="flex gap-3 my-10 justify-center items-center w-screen flex-wrap">
      {core_competencies.length > 0 ? (
        core_competencies.map((skill, index) => {
          const IconComponent = SiIcons[skill.icon];
          return (
            <div key={skill.id} className="shadow-sm rounded-md h-36 w-28 flex justify-center items-center">
              {skill.icon != "" ? (
